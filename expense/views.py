...
def login_page(request):
    if request.method == 'POST':
        ...
        if not user_obj.exists():
            messages.error(request, 'Error : Username or Email not exists')
...
def logout_page(request):
    logout(request)
    messages.error(request, 'Success : logouted successfull')
    return redirect('/login/')
...
def index(request):
    ...
    context = {'transactions' : Transaction.objects.filter(created_by = request.user),
               ...
               'income' : Transaction.objects.filter(created_by = request.user).filter(amount__gt = 0).aggregate(income = Sum('amount'))['income'] or 0,
